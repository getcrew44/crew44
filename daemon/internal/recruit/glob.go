...
// includes returns true when at least one pattern in includes matches
// candidate. An empty includes list matches nothing; callers should
// supply a default include set when the manifest omits Payload.
func includes(patterns []string, candidate string) bool {
	for _, p := range patterns {
		if matchPayloadGlob(p, candidate) {
			return true
		}
	}
	return false
