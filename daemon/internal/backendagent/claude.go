...
func writeMcpConfigToTemp(raw json.RawMessage) (string, error) {
	f, err := os.CreateTemp("", "crew44-mcp-*.json")
	if err != nil {
...
// version reaches stdout — feeding the raw concatenation into the semver
// regex then fails and the runtime is silently skipped from rescan.
//
// Heuristic: return the first non-empty line that carries a semver-shaped
// token. Strings like "2.1.5 (Claude Code)" or "codex-cli 0.118.0" survive
// unchanged because the whole matching line is returned. If no line carries a
// semver token, fall back to the trimmed raw output so unusual formats aren't
