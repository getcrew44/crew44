...
// overridden by user-configured custom_args. `--acp` puts qodercli into
// ACP-server mode; overriding it would break the daemon↔qoder protocol.
var qoderBlockedArgs = map[string]blockedArgMode{
	"--acp": blockedStandalone,
}

// qoderBackend implements Backend by spawning `qodercli --acp` and
// communicating via the standard Agent Client Protocol (ACP) JSON-RPC 2.0
// over stdin/stdout.
//
// Qoder CLI (https://qoder.com/cli, npm `@qoder-ai/qodercli`) ships a
// first-class ACP server mode that follows the same `initialize` +
// `session/new` + `session/prompt` flow as Hermes and Kimi, so we reuse
// the existing hermesClient transport. Only the binary, the activation
// flag, and the stderr label differ.
...
func (b *qoderBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// Sniff stderr for upstream-provider failure markers so we can promote
	// a session/prompt that ended with stopReason=end_turn into a proper
	// failed result when the LLM actually erred (HTTP 4xx, expired token).
	// Same rationale as kimi.go / hermes.go.
	...
	go func() {
		...
		// 3. Honour the UI's model pick (if any) before sending the prompt.
		// Failing closed on session/set_model errors keeps the user's choice
		// from being silently downgraded to the agent's default model.
		if opts.Model != "" {
		...
		// 4. Build the prompt content, prepending a system prompt when present.
		userText := prompt
