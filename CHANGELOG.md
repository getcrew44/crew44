...
### Added
- **The main interactive agent now has a headless browser.** Crew44 injects a Playwright MCP server into the runtime's isolated config (`.claude.json` for claude, `config.toml` for codex), so an agent gains `browser_navigate`, `browser_take_screenshot`, and `browser_snapshot` tools the moment it asks for them. The browser runs headless and isolated, launched on demand via `npx` — no extra binary ships with Crew44. The injection preserves any existing config (other MCP servers, settings, big integer timestamps) and is idempotent across reruns. Agents reuse one shared Chromium download via the host `ms-playwright` cache, overridable with `PLAYWRIGHT_BROWSERS_PATH`. Browser injection is opt-in per run, so background utility calls (like the chat-title summarizer, which runs on untrusted user content under bypass-permissions) get no browser surface.
