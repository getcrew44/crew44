...
func runRuntimes(ctx Context, args []string) error {
	if len(args) == 0 {
		return usageError("missing runtimes subcommand")
	}
	switch args[0] {
	case "list":
		var resp any
	...
	case "update":
		...
		return printJSON(ctx.Stdout, resp)
	default:
		return usageError("unknown runtimes subcommand %q", args[0])
	}
}

func runAgents(ctx Context, args []string) error {
	if len(args) == 0 {
		return usageError("missing agents subcommand")
	}
	switch args[0] {
	case "list":
		var resp any
	...
	case "set-skills":
		...
		return printJSON(ctx.Stdout, resp)
	default:
		return usageError("unknown agents subcommand %q", args[0])
	}
}

func runSkills(ctx Context, args []string) error {
	if len(args) == 0 {
		return usageError("missing skills subcommand")
	}
	switch args[0] {
	case "list":
		var resp any
	...
	case "delete-file":
		...
		if *id == "" || *fileID == "" {
			return usageError("missing --id or --file-id")
		}
		var resp any
		...
		return printJSON(ctx.Stdout, resp)
	default:
		return usageError("unknown skills subcommand %q", args[0])
	}
}

func runProjects(ctx Context, args []string) error {
	if len(args) == 0 {
		return usageError("missing projects subcommand")
	}
	switch args[0] {
	case "list":
		var resp any
	...
	case "chats":
		...
		return printJSON(ctx.Stdout, resp)
	default:
		return usageError("unknown projects subcommand %q", args[0])
	}
}

func runChats(ctx Context, args []string) error {
	if len(args) == 0 {
		return usageError("missing chats subcommand")
	}
	switch args[0] {
	case "list":
	...
	case "cancel":
		...
		return printJSON(ctx.Stdout, resp)
	default:
		return usageError("unknown chats subcommand %q", args[0])
	}
}
